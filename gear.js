...
function main() {
    ...
    //COPIED FROM http://learnwebgl.brown37.net/09_lights/lights_combined.html.
...
void main() {
  ...
  v_Normal = vec3( u_VM_transform * vec4(a_normal, 0.0) );
...
function drawScene(gl, programInfo, buffers, angle_x, angle_y) {
    ...
    matrix.multiplySeries(transform, rotate_x_matrix, rotate_y_matrix, scale);

    //COPIED CODE provided by instructor in announcement board.
    //Code performs ordrer of operations: modeling, interactions,
    //lookat, and then projections.
    ...
    matrix.multiplySeries(pvm_transform, proj, lookat, rotate_x_matrix, rotate_y_matrix, scale);

    const vm_transform = matrix.create();
    matrix.multiplySeries(vm_transform, lookat, rotate_x_matrix, rotate_y_matrix, scale);


    // Set the shader program's uniform
    //Added light color, shininess and ambient color to match code
    //webgl demo program.  Without light color, shininess, and ambient color
    //the gear was very dull when the front face was rendered.
    ...
    gl.uniform3f(programInfo.locations.u_Light_color, .07, .07, .07);

    gl.uniform1f(programInfo.locations.u_Shininess, 20);
